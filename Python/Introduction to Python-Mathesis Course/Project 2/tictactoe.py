...
def place_marker(board, marker, position): # 2 μονάδες
    ...
    ##< τιμη_κλειδιου = λεξικο[κλειδί] >
    #for position in theBoard:
        #if board[position]==' ':
...
def player_choice(theBoard, turn): # 2 μονάδες *
    ...
    position=' '
    while position not in '1 2 3 4 5 6 7 8 9'.split(): #or not board_check(board):
        print('Ποιά θέση επιλέγεις; (1-9)')
        position=input()
    return int(position)
